...
int data_debug(PARA_DATA *para, REAL *var, char *name) {
  ...
  fprintf(datafile, "%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n",
    var[AW*size + i], var[AE*size + i], var[AS*size + i], var[AN*size + i],
    var[AB*size + i], var[AF*size + i], var[AP0*size + i], var[AP*size + i], 
    var[B*size + i], var[TEMP*size + i]);
    }
  ...
  ****************************************************************************/
  ...
  FOR_ALL_CELL
    ...
    fprintf(datafile, "%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n",
            um[IX(i,j,k)], vm[IX(i,j,k)], wm[IX(i,j,k)], Tm[IX(i,j,k)],
            Xi[IX(i,j,k)], flagp[IX(i,j,k)], p[IX(i,j,k)], sqrt(um[IX(i,j,k)]*um[IX(i,j,k)]+vm[IX(i,j,k)]*vm[IX(i,j,k)]+wm[IX(i,j,k)]*wm[IX(i,j,k)]));
  END_FOR
...
int write_SCI(PARA_DATA *para, REAL **var, char *name) {
  ...
  ****************************************************************************/
  ...
  return 0;
	...
	///////////////////////////////////////////////////////////////////////////////
	/// Write standard output data in a format for paraview in VTK (Visualization Toolkit)
...
int write_vtk_data(PARA_DATA *para, REAL **var, char *name) {
		...
		****************************************************************************/
		...
		fprintf(datafile, "LOOKUP_TABLE default\n");
		FOR_ALL_CELL
				fprintf(datafile, "%f\t", sqrt(u[IX(i, j, k)] * u[IX(i, j, k)] + v[IX(i, j, k)] * v[IX(i, j, k)] + w[IX(i, j, k)] * w[IX(i, j, k)]));
		END_FOR
				fprintf(datafile, "\n");
		...
		FOR_ALL_CELL
				fprintf(datafile, "%f\t%f\t%f\n", u[IX(i, j, k)], v[IX(i, j, k)], w[IX(i, j, k)]);
		END_FOR

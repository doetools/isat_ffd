...
int read_parameter(PARA_DATA *para) {
  ...
  ****************************************************************************/
  ...
  if(para->solv->cosimulation==0) {
    if((file_para_reader=fopen("input.ffd","r"))==NULL) {
  ...
  ---------------------------------------------------------------------------*/
    ...
    if((file_para_reader=fopen(para->cosim->para->fileName,"r"))==NULL) {
  ...
  while(fgets(string, 400, file_para_reader) != NULL) {
  ...
  if (!feof(file_para_reader)){
      sprintf(msg, "read_parameter(): Could not read data from file %s",
            para->cosim->para->fileName);
      ffd_log(msg, FFD_ERROR);
  }

  fclose(file_para_reader);
  return 0;

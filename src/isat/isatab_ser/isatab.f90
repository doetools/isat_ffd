...
recursive subroutine isatab( idtab, mode, nx, x, nf, nh, nhd, usrfgh1, iusr, rusr, &
...
!---------------------------------------------------------------------------
   ...
   integer, intent(in) :: mode, idtab, nx, nf, nh, nhd, info(l_info), usrfgh1
   integer             :: iusr(*)
...
!      f_evaluated = .true.
       ...
       if( .not. ( f_evaluated  .and.  need_g ==0  .and.  need_h == 0 )) then  !  need to evaluate g and h
          need = (/ 1, need_g, need_h /)  !  obtain g and h 
          if( f_evaluated ) need(1) = 0
          !if( f_evaluated ) print*, "**********successful avoid"
          call isatab_set_fe
          xe = x
